...
*Amended again at the user's direction, after a trial of both (the antechamber, one large
space):* the view follows the player inside a room -- leading the way they run, holding
still through jumps, moving when they land higher or lower, never showing past the room's
edges, in whole pixels. The slide between screens read as entering a new room inside the
same one, so it is gone; a cut is kept for real thresholds between areas (a door). What
the locked frame asked for is still asked of every screen-sized part of a room: each is
composed as a screen (the tools shoot the six that way).
