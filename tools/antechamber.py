...
claude/ANTECHAMBER.md; this is only where things are.
    ...
    D the door, the camp         | E the keeper's lap   | F the stair
                                   (and behind B and E, the view)

The antechamber is a balcony: its far wall is open behind the keeper on the rest of the
archive, a chamber miles across (tools/art/vista.py, src/city.c). The door is at the left
end, the window at the right. You wake at the door's foot. Rows that matter: the floor is
...
# the basin: level with the floor, under the keeper's shins and along the parapet, stepping
...
# ------------------------------------------------------------------ zones
# The raw rock is the mountain at the door's end: its wall, floor and roof. Everything else
# is theirs.
R.city(36, 0, 119, 43)

# ------------------------------------------------------------------ the openings
# The far wall is one picture (tools/art/archive.py); these are the holes in it (x, y, w, h
# in tiles): the window's cell, a circle in its square, and the great opening behind the
# keeper, between the piers and above the parapet. archive.py cuts the picture to them.
R.feat('F_WINDOW', 94, -1, 21, 21)
R.feat('F_VISTA', 36, 1, 51, 36)
