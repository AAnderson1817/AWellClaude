param([string]$OutputDirectory='docs/evidence/architecture/native')
...
foreach($mode in @('with-occlusion','without-occlusion')) {
    $flags=@()
...
$hashes=@{}
foreach($path in @('src/depth.c','src/generated/terrain_assets.h','src/generated/support_assets.h','src/generated/foliage_assets.h')){$hashes[$path]=(Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLower()}
[ordered]@{capturedUtc=(Get-Date).ToUniversalTime().ToString('o');sourceHashes=$hashes;scope='Same source/geometry, AO build control only; actual native images and local frame-loop wall timing. Not GPU-only timing or hardware-wide performance acceptance.'} | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath (Join-Path $folder 'manifest.json') -Encoding utf8
