...
"${AWELL_CC[@]}" -std=c99 -O1 -g -DAWELL_HEADLESS -fsanitize=undefined -fno-sanitize-recover=all \
  ...
  src/player.c src/room.c src/fx.c src/render.c src/audio.c src/life.c src/items.c src/props.c src/city.c -lm -o "$check_dir/regressions"
"$check_dir/regressions"
"${AWELL_CC[@]}" -std=c99 -O1 -g -DAWELL_HEADLESS -fsanitize=undefined -fno-sanitize-recover=all \
  -I"$RAYLIB" tools/tests/presentation_snapshots.c tools/tests/raylib_stubs.c \
  src/player.c src/room.c src/fx.c src/render.c src/audio.c src/life.c src/items.c src/props.c src/city.c -lm -o "$check_dir/snapshots"
"$check_dir/snapshots"
"${AWELL_CC[@]}" -std=c99 -O1 -g -DAWELL_HEADLESS -fsanitize=undefined -fno-sanitize-recover=all \
  -I"$RAYLIB" tools/tests/city_responses.c tools/tests/raylib_stubs.c \
  src/player.c src/room.c src/fx.c src/render.c src/audio.c src/life.c src/items.c src/props.c src/city.c -lm -o "$check_dir/city"
"$check_dir/city"
